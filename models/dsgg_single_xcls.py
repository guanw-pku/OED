...
from .cdn_single_xcls import build_cdn
from models.word_vectors import obj_edge_vectors, predicate_edge_vectors
from util.label_set import OBJ_CLASSES, PREDICATE_CLASSES
...
class CDNDSGG(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_queries, aux_loss=False, args=None):
        ...
        self.semantic_head = args.semantic_head
        if self.semantic_head:
            obj_semantic_embed = obj_edge_vectors(OBJ_CLASSES, wv_type='glove.6B', wv_dir='./data/', wv_dim=200)
            pred_semantic_embed = predicate_edge_vectors(PREDICATE_CLASSES, wv_type='glove.6B', wv_dir='./data/', wv_dim=200)
        ...
        if self.dsgg_task == 'sgcls':
            self.sub_class_embed = nn.Linear(hidden_dim, 2) # whether or not
        # if not self.one_dec:
        self.sub_bbox_embed = MLP(hidden_dim, hidden_dim, 4, 3)
        self.obj_bbox_embed = MLP(hidden_dim, hidden_dim, 4, 3)

        if not self.semantic_head:
            ...
            self.obj_class_embed = nn.Linear(200, num_obj_classes + 1)
            self.attn_class_embed = nn.Linear(200, args.num_attn_classes + 1) # add 1 for background
            self.spatial_class_embed = nn.Linear(200, args.num_spatial_classes)
            self.contacting_class_embed = nn.Linear(200, args.num_contacting_classes)

            self.obj_class_embed.weight.data[:num_obj_classes] = obj_semantic_embed
            self.attn_class_embed.weight.data[:args.num_attn_classes] = pred_semantic_embed[:args.num_attn_classes]
            self.spatial_class_embed.weight.data[:args.num_spatial_classes] = pred_semantic_embed[args.num_attn_classes:args.num_attn_classes+args.num_spatial_classes]
            self.contacting_class_embed.weight.data[:args.num_contacting_classes] = pred_semantic_embed[args.num_attn_classes+args.num_spatial_classes:]

            self.obj_class_embed = nn.Sequential(nn.Linear(hidden_dim, 200), self.obj_class_embed)
            self.attn_class_embed = nn.Sequential(nn.Linear(hidden_dim, 200), self.attn_class_embed)
            self.spatial_class_embed = nn.Sequential(nn.Linear(hidden_dim, 200), self.spatial_class_embed)
            self.contacting_class_embed = nn.Sequential(nn.Linear(hidden_dim, 200), self.contacting_class_embed)
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        if self.one_dec:
            hopd_out, valid_num_list = self.transformer(self.input_proj(src), mask, pos[-1], targets)
            interaction_decoder_out = hopd_out
        else:
            hopd_out, interaction_decoder_out, valid_num_list = self.transformer(self.input_proj(src), mask, pos[-1], targets)

        out = {}
        if self.dsgg_task == 'sgcls':
            outputs_sub_class = self.sub_class_embed(hopd_out)
        ...
        if not self.one_dec:
            ...
            out.update({'pred_obj_logits': outputs_obj_class[-1], 'pred_sub_boxes': outputs_sub_coord[-1], 'pred_obj_boxes': outputs_obj_coord[-1]})
        ...
        out.update({'pred_attn_logits': outputs_attn_class[-1], 'pred_spatial_logits': outputs_spatial_class[-1], 
                    'pred_contacting_logits': outputs_contacting_class[-1], 'valid_num_list': valid_num_list})
            ...
            if self.dsgg_task == 'predcls':
                if not self.one_dec:
                    out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_sub_coord, outputs_obj_coord,
                                                    ...
                                                    outputs_contacting_class, valid_num_list)
                else:
                    out['aux_outputs'] = self._set_aux_loss_rel(outputs_attn_class, outputs_spatial_class,
                                                                outputs_contacting_class, valid_num_list)
            else:
                out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_sub_coord, outputs_obj_coord,
                                                    outputs_attn_class, outputs_spatial_class,
                                                    outputs_contacting_class, valid_num_list, outputs_sub_class)

        return out

    @torch.jit.unused
    def _set_aux_loss(self, outputs_obj_class, outputs_sub_coord, outputs_obj_coord, outputs_attn_class, outputs_spatial_class, outputs_contacting_class, valid_num_list, outputs_sub_class=None):
        ...
        if outputs_sub_class is None:
            return [{'pred_obj_logits': a, 'pred_sub_boxes': b, 'pred_obj_boxes': c, 'pred_attn_logits': d, \
                        'pred_spatial_logits': e, 'pred_contacting_logits': f, 'valid_num_list': valid_num_list}
            ...
            return [{'pred_sub_logits': a, 'pred_obj_logits': b, 'pred_sub_boxes': c, 'pred_obj_boxes': d, 'pred_attn_logits': e, \
                        'pred_spatial_logits': f, 'pred_contacting_logits': g, 'valid_num_list': valid_num_list}
                    for a, b, c, d, e, f, g in zip(outputs_sub_class[-min_dec_layers_num : -1], outputs_obj_class[-min_dec_layers_num : -1], \
                                                    outputs_sub_coord[-min_dec_layers_num : -1], outputs_obj_coord[-min_dec_layers_num : -1], \
                                                    outputs_attn_class[-min_dec_layers_num : -1], outputs_spatial_class[-min_dec_layers_num : -1], \
                                                    outputs_contacting_class[-min_dec_layers_num : -1])]

    def _set_aux_loss_rel(self, outputs_attn_class, outputs_spatial_class, outputs_contacting_class, valid_num_list):
        min_dec_layers_num = min(self.dec_layers_hopd, self.dec_layers_interaction)
        return [{'pred_attn_logits': a, 'pred_spatial_logits': b, 'pred_contacting_logits': c, 'valid_num_list': valid_num_list}
                for a, b, c in zip(outputs_attn_class[-min_dec_layers_num : -1], outputs_spatial_class[-min_dec_layers_num : -1], \
                                   outputs_contacting_class[-min_dec_layers_num : -1])]
...
class SetCriterionDSGG(nn.Module):
    ...
    def __init__(self, num_obj_classes, num_queries, matcher, weight_dict, eos_coef, losses, args):
        ...
        attn_empty_weight = torch.ones(self.num_attn_classes + 1)
        attn_empty_weight[-1] = self.eos_coef
        self.register_buffer('attn_empty_weight', attn_empty_weight)
        ...
        self.attn_nums_init.append(3 * sum(self.attn_nums_init))
        self.spat_nums_init.append(3 * sum(self.spat_nums_init))
        self.cont_nums_init.append(3 * sum(self.cont_nums_init))
    ...
    def loss_sub_labels(self, outputs, targets, indices, num_interactions, log=True):
        ...
        if self.dsgg_task == 'predcls':
            ...
            valid_src_logits = torch.cat([src_logits[i:i+1, :valid_num_list[i]] for i in range(len(valid_num_list))], dim=1)
    ...
    def loss_relation_labels(self, outputs, targets, indices, num_interactions):
        ...
        if not self.rel_reweight:
            attn_weights = self.attn_empty_weight
            spat_weights = None
            cont_weights = None
        elif self.use_static_weights:
            attn_weights = self.attn_weights_init.to(target_attn_classes_o.device)
            spat_weights = self.spat_weights_init.to(target_spatial_classes_o.device)
            cont_weights = self.cont_weights_init.to(target_contacting_classes_o.device)
        else:
            attn_nums_in_batch = torch.tensor([0] * (self.num_attn_classes + 1))
            spat_nums_in_batch = torch.tensor([0] * (self.num_spatial_classes + 1))
            cont_nums_in_batch = torch.tensor([0] * (self.num_contacting_classes + 1))
            for target in targets:
                attn_nums_in_batch += torch.cat([target['attn_labels'].long().sum(dim=0).cpu(), torch.zeros(1).long()])
                spat_nums_in_batch += torch.cat([target['spatial_labels'].long().sum(dim=0).cpu(), torch.zeros(1).long()])
                cont_nums_in_batch += torch.cat([target['contacting_labels'].long().sum(dim=0).cpu(), torch.zeros(1).long()])
            if self.q_attn.full(): self.q_attn.get()
            if self.q_spat.full(): self.q_spat.get()
            if self.q_cont.full(): self.q_cont.get()
            self.q_attn.put(np.array(attn_nums_in_batch))
            self.q_spat.put(np.array(spat_nums_in_batch))
            self.q_cont.put(np.array(cont_nums_in_batch))
            accumulated_attn_nums = np.sum(self.q_attn.queue, axis=0)
            accumulated_spat_nums = np.sum(self.q_spat.queue, axis=0)
            accumulated_cont_nums = np.sum(self.q_cont.queue, axis=0)
            attn_weights = self.cal_weights(accumulated_attn_nums, p=self.p_rel)
            spat_weights = self.cal_weights(accumulated_spat_nums, p=self.p_rel)
            cont_weights = self.cal_weights(accumulated_cont_nums, p=self.p_rel)
        ...
        loss_attn_ce = F.cross_entropy(attn_logits, target_attn_labels, weight=attn_weights)

        loss_spatial_ce = self._neg_loss(spatial_probs, target_spatial_classes, weights=spat_weights, alpha=self.alpha)
        loss_contacting_ce = self._neg_loss(contacting_probs, target_contacting_classes, weights=cont_weights, alpha=self.alpha)
    ...
    def forward(self, outputs, targets):
        ...
        if self.dsgg_task == 'predcls':
            ...
            for tid in range(len(targets)):
                ...
                num_boxes = len(targets[tid]['boxes'])
                sub_idx = torch.where(targets[tid]['labels'] == 1)[0]
                pair_indices = torch.tensor(list(itertools.permutations(range(num_boxes), 2)), device=sub_idx.device)
                valid_pair_indexes = torch.where(pair_indices[:, 0] == sub_idx)[0]
                indices.append((valid_pair_indexes.cpu(), torch.arange(len(targets[tid]['obj_labels']))))
    ...
    def forward(self, outputs, targets):
        ...
        for index in range(len(target_sizes)):
            ...
            if self.dsgg_task == 'predcls':
                ...
                frame_pred['target_labels'] = torch.cat([torch.ones(num_valid_query), targets[index]['obj_labels']], dim=0).cpu().numpy()
                frame_pred['target_boxes'] = torch.cat([targets[index]['sub_boxes'], targets[index]['obj_boxes']], dim=0).cpu().numpy()
            else:
                frame_pred['pred_boxes'] = torch.cat([frame_sub_boxes, frame_obj_boxes], dim=0).cpu().numpy()
