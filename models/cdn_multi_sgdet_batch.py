...
class CDN(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, class_embed_dict=None, targets=None, cur_idx=0):
        ...
        ###########
        extend_src = src.repeat(1, bs, 1)
        extend_mask = mask.repeat(1, bs, 1)
        extend_pos_embed = pos_embed.repeat(1, bs, 1)
        ...
        ##############
        extend_tgt = tgt.repeat(1, bs, 1)
        extend_memory = memory.repeat(1, bs, 1)
        extend_mask = mask.repeat(bs, 1)
        extend_pos_embed = pos_embed.repeat(1, bs, 1)
        extend_query_embed = query_embed.repeat(1, bs, 1)
        hopd_out = self.decoder(extend_tgt, extend_memory, memory_key_padding_mask=extend_mask, \
                                pos=extend_pos_embed, query_pos=extend_query_embed)
        hopd_out = hopd_out[:, :, :6]
        ##############
        ...
        #################
        extend_interaction_tgt = interaction_tgt.repeat(1, bs, 1)
        extend_interaction_query_embed = interaction_query_embed.repeat(1, bs, 1)
        interaction_decoder_out = self.interaction_decoder(extend_interaction_tgt, extend_memory, memory_key_padding_mask=extend_mask,
                                        pos=extend_pos_embed, query_pos=extend_interaction_query_embed) # 2d pe as no temopral interaction in spatial decoder
        interaction_decoder_out = interaction_decoder_out[:, :, :6]
        ################
        ...
        if self.query_temporal_interaction:
            ...
            ##################
            ref_ins_hs = ref_ins_hs.repeat(bs, 1, 1)
            ref_rel_hs = ref_rel_hs.repeat(bs, 1, 1)
            ...
            ref_obj_prob = ref_obj_prob[:1]
            ref_attn_prob = ref_attn_prob[:1]
            ref_spatial_prob = ref_spatial_prob[:1]
            ref_contacting_prob = ref_contacting_prob[:1]
            #################
            ...
            if self.one_temp:
                ...
                if self.num_dec_layers_temporal == 1:
                    # pdb.set_trace()
                    #########################
                    extend_cur_concat_hs = cur_concat_hs.repeat(bs, 1, 1)
                    extend_ref_concat_hs_input1 = ref_concat_hs_input1.repeat(bs, 1, 1)
                    cur_concat_hs = self.temporal_query_layer1(extend_cur_concat_hs, extend_ref_concat_hs_input1, cur_hs_tpe, ref_hs_tpe_topk)
                    cur_concat_hs = cur_concat_hs[:1]
                    ########################
